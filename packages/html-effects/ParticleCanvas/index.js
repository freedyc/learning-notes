...
let mouseBody = new Body(new Vector(0, 0), new Vector(0, 0), -1000, 10, true);
let bodyIndex = null;
...
// sim.addBody(new Body(new Vector(0, 0), new Vector(0, 0), 0, 50, true));

sim.addBody(new Body(new Vector(0, 0), new Vector(0, 0), 0, 20, true));

for (i = 0; i < 100; i++) {
    const r = new Vector(200 + Math.random() * 1024, Math.random() * Math.PI * 2);
    const p = new Vector(Math.cos(r.y) * r.x, Math.sin(r.y) * r.x);
    const a = new Vector(p.y, -p.x).normalise().scale(10);
    // const a = new Vector(0,0);
    const rf = Math.random();
    sim.addBody(new Body(p, a, -10 + rf * 15, rf * 15));
